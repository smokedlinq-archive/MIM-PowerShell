...
namespace MIM.PowerShell
{
    public class PSTraceSource : TraceSource
    {
            ...
            : base(name)
        {
            try
            {
                var key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Forefront Identity Manager\2010\Synchronization Service\MIM.PowerShell");
                var path = (string)key.GetValue("Path", string.Empty);

                this.Switch.Level = SourceLevels.Information;
                ...
                if (!string.IsNullOrWhiteSpace(path))
                {
                    ...
                    this.Listeners.Add(new XmlWriterTraceListener(Path.Combine(path, "MIM.PowerShell.svclog"), "PSTraceSourceXmlWriterTraceListener"));
                }

                this.Listeners.Add(new EventLogTraceListener("MIM.PowerShell")
                {
