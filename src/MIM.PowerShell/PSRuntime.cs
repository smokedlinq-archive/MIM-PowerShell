...
namespace MIM.PowerShell
{
    public abstract class PSRuntime : IDisposable
    {
        ...
        protected void Initialize()
        {
            try
            {
                using (TraceSource.TraceMethod())
                {
                    ...
                    //   MIM can get 'upset' over other threads running off to do things
                    this._runspace = RunspaceFactory.CreateRunspace();
                    this._runspace.ThreadOptions = PSThreadOptions.UseCurrentThread;

                    this._runspace.Open();

                    // Load the extension profile script MIM.PowerShell.ps1
            ...
            {
                TraceSource.TraceException(TraceEventType.Critical, ex, "Initialize method failed with error: {0}", ex.Message);
                throw;
            }
        }
        ...
        IEnumerable<string> Profile
        {
            get
            {
                yield return Path.GetFullPath(Path.Combine(Environment.ExpandEnvironmentVariables(@"%SYSTEMROOT%\system32\WindowsPowerShell\v1.0"), "MIM.PowerShell.ps1"));
                
                if (!string.IsNullOrWhiteSpace(ExtensionsDirectory))
                    yield return Path.GetFullPath(Path.Combine(ExtensionsDirectory, "MIM.PowerShell.ps1"));

                if (!string.IsNullOrWhiteSpace(MAFolder))
                    yield return Path.GetFullPath(Path.Combine(MAFolder, "MIM.PowerShell.ps1"));
            }
        }
        ...
        Collection<T> Invoke<T>(PSCommand commands, Func<System.Management.Automation.PowerShell, Collection<T>> invoker)
        {
            ...
            {
                TraceSource.TraceException(TraceEventType.Critical, ex, "Invoke method failed with error: {0}", ex.Message);
                throw;
            }
        }
